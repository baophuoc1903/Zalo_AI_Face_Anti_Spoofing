...
class AntiSpoofingModel(torch.nn.Module):
    def __init__(self, n_class=1, eff=False):
        super().__init__()
        self.eff = eff
        self.n_class = n_class
        if eff:
            self.backbone = timm.create_model('efficientnet_b0', pretrained=True)
            self.cls = self.backbone.classifier.in_features
            self.backbone.classifier = nn.Linear(self.cls, n_class)
        else:
            self.backbone = FeatherNetB()
            self.cls = 128
            self.backbone = torch.nn.Sequential(*[self.backbone,
                                                  torch.nn.Linear(self.cls, n_class)])
    ...
    def _initialize_weights(self):
        if self.eff:
            self.backbone = timm.create_model('efficientnet_b0', pretrained=True)
            self.cls = self.backbone.classifier.in_features
            self.backbone.classifier = nn.Linear(self.cls, self.n_class)
        else:
...
if __name__ == '__main__':
    model = AntiSpoofingModel(1, eff=False)

    from torchsummary import summary
    from dataset.create_dataset import FAS_Dataset, transform_image
    ...
    dataset = FAS_Dataset(df,
                          ...
                          transforms=partial(transform_image, task='valid'))
    ...
    preds = model(images)
    print(compute_eer(labels.detach().numpy(), preds.detach().numpy(), positive_label=1))
